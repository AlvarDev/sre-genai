...
1. **Serving Performance**:
   - **TTFT (Time-to-First-Token)** & **TPOT (Time-per-Output-Token)** from Direct Core Model tests (Test 3 & Test 4 via `/v1/core`).
   - **End-to-End Latency (E2EL)** from Full Pipeline tests (Test 1 & Test 2 via `/v1`).
